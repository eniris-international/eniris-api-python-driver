...
class PointPrinter(PointWriter):
  """A PointWriter which prints any received points. Helpful for development and demonstration purposes"""
...
class PointWriterDecorator(PointWriter):
  ...
  def flush(self):
    """Flush any internal state, i.e. make sure that any internally stored or buffered points are transmitted and then flush the output"""
    self._flush()
    self.output.flush()
  
  def _flush(self):
    """A placeholder function which may be overridden in child classes. The overriding method only has to make sure that internally stored points are transmitted."""
    pass
