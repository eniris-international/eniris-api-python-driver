...
@dataclass
class Namespace:
  """A namespace in which measurements get stored. A namespace determines who can access measurements and the retention of the data stored in the namespace.
  
  All Influx versions have incorporated this concept in one way or another:
  - In InfluxDB 1 the namespace is determined by both the database and the retention policy of a measurement, see: https://docs.influxdata.com/influxdb/v1.8/concepts/glossary/#retention-policy-rp
  - In InfluxDB 2 the namespace is determined by the organization and the bucket of a measurement, see: https://docs.influxdata.com/influxdb/v2.0/organizations/buckets/
  - In InfluxDB 3 the namespace is specified directly using a single string, see: https://github.com/influxdata/influxdb_iox#use-influxdb-20-api-compatibility

  Attaching a namespace abstracts which InfluxDB version is used for storage and thus allows to construct points in a consistent way over all versions.
  """
  ...
  def fromJson(**kwargs):
    ...
    Returns:
      V1Namespace|V2Namespace|V3Namespace: The namespace corresponding to the passed keyword arguments
  ...
  def toUrlParameters(self) -> 'dict[str, str]':
    """A method which returns which url parameters should be attached to a POST request which is storing data for a namespace
    
    Returns:
      dict[str, str]: The url parameters required to store data in the specified namespace
    """
    raise NotImplementedError("This method should be overrriden in child classes")
  
  def toJson(self):
    """A JSON dumpable representation of the Namespace object, which can be converted back into the object using the Namespace.fromJson method
    """
    return asdict(self)

@dataclass
class V1Namespace(Namespace):
  """The InfluxDB 1 implementation of the namespace concept, i.e. the combination of a 'database' and 'retentionPolicy'
  """
  database: str
  retentionPolicy: str
  ...
  def validateRetentionPolicy(retentionPolicy: str) -> None:
    ...
    Returns:
        None: An exception is raised when the argument is not a valid retention policy name
    """
...
class V2Namespace(Namespace):
  """The InfluxDB 2 implementation of the namespace concept, i.e. the combination of an 'organization' and a 'bucket'
  """
  organization: str
  bucket: str
  ...
  def validateOrganization(organization: str) -> None:
    ...
    Returns:
        None: An exception is raised when the argument is not a valid organization name
    """
