...
class Telemessage(object):
  ...
  def __init__(self, parameters: 'dict[str, str]', lines: 'list[bytes]'):
    ...
    Args:
        params (dict[str, str]): Query parameters which should be added to the post request in which the telemessage lines are transmitted
        lines (list[bytes]): A list of bytes, where each byte is a lineprotocol encoded point
    """
